    ...
    <div class="container">
                        ...
                        <optgroup label="{{ $faculty->name }}">
                            @foreach($faculty->studies as $study)
                                <option value="{{ $study->id }}"
                                        @if(request()->has('study_id') && request()->get('study_id') == $study->id) selected @endif>{{ $study->name }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
        ...
        <div class="card">
            <div class="card-header bg-umy d-flex flex-row justify-content-between align-items-center align-middle">
                <span class="d-block">{{ __('Laporan Unit') }}</span>
                <form action="{{ route('report.unit') }}" method="get">
                    ...
                    <table class="table table-hover table-bordered">
                        ...
                        @forelse($submissions as $submission)
                            <tr>
                                ...
                                <td style="width:150px">{{ $submission['date'] }}</td>
                                <td style="width:150px" class="text-right">Rp {{ number_format($submission['cost'], 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td class="text-center" colspan="4">Tidak ada pengajuan</td>
                            </tr>
                        @endforelse
                        </tbody>
                        <tfoot>
                        <tr>
                            <th scope="col" colspan="3" class="text-right">{{ __('Biaya') }}</th>
