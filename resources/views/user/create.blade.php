                    ...
                    <div class="card-body">
                        <form action="{{ route('user.store') }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" required name="name" id="name" class="form-control @error('name') is-invalid @enderror" autocomplete="off" autofocus value="{{ old('name') }}">
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <input type="email" required name="email" id="email" class="form-control @error('email') is-invalid @enderror" autocomplete="off" value="{{ old('email') }}">
                                @error('email')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="role" id="role" class="form-control @error('role') is-invalid @enderror" required @change="roleOnChange($event.target.value)">
                                    @foreach(\App\User::getModel()->roles as $role => $name)
                                        <option value="{{ $role }}" @if(old('role') == $role) selected @endif>{{ $name }}</option>
                                    @endforeach
                                </select>
                                @error('role')
                                <span class="invalid-feedback">{{ $message }}</span>
