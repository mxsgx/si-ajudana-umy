        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="d-flex justify-content-between mb-4">
                    <a class="btn btn-outline-secondary" href="{{ route('submission.index') }}">&laquo; {{ __('Kembali') }}</a>
                </div>
                <div class="card">
                        ...
                        <form action="{{ route('submission.store') }}" method="post" enctype="multipart/form-data">
                            ...
                            @if(auth()->user()->role === 'admin')
                                <div class="form-group">
                                    ...
                                    <select name="lecturer_id" id="lecturer_id" required class="form-control @error('lecturer_id') is-invalid @enderror">
                                        @foreach(\App\Lecturer::all(['id', 'name']) as $lecturer)
                                            <option value="{{ $lecturer->id }}" @if(old('lecturer_id') == $lecturer->id) selected @endif>{{ $lecturer->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('lecturer_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" required class="form-control @error('lecturer_id') is-invalid @enderror">
                                        @foreach(\App\Submission::getModel()->statuses as $status => $name)
                                            <option value="{{ $status }}" @if(old('status') == $status) selected @endif>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    @error('status')
                                    <span class="invalid-feedback">{{ $message }}</span>
                            ...
                            <div class="form-group">
                                ...
                                <select name="activity_id" id="activity_id" required class="form-control @error('activity_id') is-invalid @enderror">
                                ...
                                <div class="col-lg-6 col-md-12">
                                    <div class="row">
                                        <div class="col-md-7 form-group">
                                            ...
                                            <input type="date" name="date_start" id="date_start" required class="form-control @error('date_start') is-invalid @enderror" value="{{ old('date_start') }}">
                                            @error('date_start')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="place" id="place" required class="form-control @error('place') is-invalid @enderror" value="{{ old('place') }}">
                                @error('place')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group card card-body">
                                ...
                                @foreach(\App\Category::all(['id', 'name']) as $category)
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" id="category-{{ $category->id }}" name="category_id" class="custom-control-input" value="{{ $category->id }}" required @if(old('category_id') == $category->id) checked @endif>
                            ...
                            <div class="form-group card card-body">
                                ...
                                @foreach(\App\Participant::all(['id', 'name']) as $participant)
                                    <div class="custom-control custom-checkbox custom-control-inline">
                                        <input type="checkbox" id="participant-{{ $participant->id }}" name="participant_id[]" class="custom-control-input" value="{{ $participant->id }}" @if(in_array($participant->id, old('participant_id', []))) checked @endif>
                                        <label class="custom-control-label" for="participant-{{ $participant->id }}">{{ $participant->name }}</label>
                                    </div>
                                @endforeach
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="writer" id="writer" required class="form-control @error('writer') is-invalid @enderror" value="{{ old('writer') }}">
                                @error('writer')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="schema" id="schema" class="form-control @error('schema') is-invalid @enderror" value="{{ old('schema') }}">
                                @error('schema')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="grant" id="grant" required class="form-control @error('grant') is-invalid @enderror" value="{{ old('grant') }}">
                                @error('grant')
                                <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                                ...
                                <div class="d-md-table w-100" id="financial-area">
                                        ...
                                        <div class="d-md-table-row financial-item">
                                            <div class="text-left d-md-table-cell p-2">
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" id="financial-{{ $financial->id }}" name="financial_id[]" class="custom-control-input" value="{{ $financial->id }}" @if(in_array($financial->id, old('financial_id', []))) checked @endif>

                                                    <label class="custom-control-label" for="financial-{{ $financial->id }}">{{ $financial->name }}</label>
                                                </div>
                                            </div>
                                            ...
                                            <div class="d-md-table-cell p-2">
                                                <div class="input-group">
                                                    <div class="input-group-prepend">
                                                        <label for="financial-value-{{ $financial->id }}" class="d-block m-0 input-group-text">Rp</label>
                                                    </div>

                                                    <input id="financial-value-{{ $financial->id }}" type="number" class="form-control" name="financial_value[{{ $financial->id }}]" value="{{ !empty(old('financial_value', [])) ? old('financial_value')[$financial->id] : 0 }}" min="0">
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="d-md-table-row">
                                        <div class="text-md-center text-left d-md-table-cell p-2">
                                            <label class="m-0 font-weight-bold text-md-center" for="finance-value-total">Total</label>
                                        </div>

                                        <div class="d-md-table-cell p-2">
                                            <div class="input-group">
                                                <div class="input-group-prepend">
                                                    <label for="financial-value-total" class="input-group-text">Rp</label>
                                                </div>

                                                <input id="financial-value-total" type="number" class="form-control" value="0" min="0" disabled="">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    @foreach(\App\Attachment::all(['id', 'name']) as $attachment)
                                        <div class="d-md-table-row">
                                            <div class="text-left d-md-table-cell p-2 align-middle">
                                                <label class="m-0" for="attachment-{{ $attachment->id }}">{{ $attachment->name }}</label>
                                            </div>

                                            <div class="d-md-table-cell p-2 align-middle">
                                                <input type="file" class="form-control-file" id="attachment-{{ $attachment->id }}" name="attachment[{{ $attachment->id }}]" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword,application/vnd.ms-excel,application/pdf,image/png,image/jpeg,image/jpeg,image/jpeg">
                                            </div>
                                        </div>

                                        @error('attachment.' . $attachment->id)
                                        <div class="d-md-table-row">
                                            <div class="text-left d-md-table-cell py-md-0 pb-md-2 px-2 align-middle"></div>
